# MSTeams-WebBotSSOTab

# Description

In this demo, we showcased how to host a web chat bot inside a MS Teams as a personal tab. it also integrated with MS Teams SSO to authenticate the chat bot. 

# Project contents: 
* bot: this is folder implemented a web bot via Bot framework
* app1: a SPA demonstrated how to use react web chat control to integrate SPA SSO with bot. 
* app3: a simple SP demonstrated how to integrate SPA authentication with bot SSO
* ReactTab01: a MS Teams personal tab demonstrated how to embed web chat bot inside a personal tab and also use MS Teams SSO for Bot SSO. 
...
-  for API permissions, included api://[bot]/[permission] and also do admin consent. 
...
* run simple SPA sample: 
  * under `app1/app3` and `bot` folder, run the following commands: 
    ```
    npm install
    npm start
    ngrok http 3978 --subdomain=[domain] 
    ``` 
  * create .env file and inlcude `REACT_APP_WEBBOTSECRET` to your bot secret
* run MS Team personal tab sample: 
  * under `bot` folder, run the following commands: 
    ```
    npm install
    npm start
    ngrok http 3978 --subdomain=[domain] 
    ``` 
  * under `ReactTab01`, hit F5. MS Teams toolkit will automatic start. if you leverage MS Teams Toolkit to create Entra applicaiton, make sure you add bot api://[123]/[permissions] permissions to created Entra application and do admin consent. 
  ![MS Teams tab demo](./docs/2024-08-30_10-54-26.gif "MS Teams tab demo") 
...
ngrok http 3978 --subdomain=ezcode

c:\tools\ngrok\ngrok http 3978 --subdomain=ezcode